...
from .models import User_Reg, Farmer_Reg, FarmHouse_Reg,Category,Product,cart,cart_items,delivery_agent,orders,govt_policies\
...
def get_products(request, uid, id):
    if request.session.get('session_id'):
        ...
        try:
            ...
            # if the page is out of range, deliver the last page
            page_obj = paginator.page(paginator.num_pages)
        return render(request, "User/view_products.html", {'page_obj': page_obj, 'login_id': uid, 'categories':categories})
    else:
        return redirect('/Login/')
...
def add_to_cart(request,uid,products_id):
    if request.session.get('session_id'):
        ...
        try:
            ...
            if carts:
                cart_item = cart_items.objects.create(product_id=products,carts_id=carts,user_id=customers,farmers_id=products.Farmer_Id)
                count = carts.count+1
...
def product_more_details(request, uid, id):
    if request.session.get('session_id'):
        products = Product.objects.get(Product_Id =id)
        categories=get_category()
        return render(request, "User/product_more_details.html",{'products': products, 'login_id': uid,'categories':categories})
    else:
        return redirect('/Login/')
    ...
    if request.session.get('session_id'):
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                image = form.files['image']
                farmhouse_id = FarmHouse_Reg.objects.get(FarmHouse_Id=uid)
                rental_product.objects.create(categories=categories, name=name, description=description, price=price,image=image,
...
def get_machinery(request, uid, id):
    if request.session.get('session_id'):
        ...
        try:
            ...
            # if the page is out of range, deliver the last page
            page_obj = paginator.page(paginator.num_pages)
        return render(request, "Farmer/view_machinery.html", {'page_obj': page_obj, 'login_id': uid, 'category':category})
    else:
        return redirect('/Login/')
...
def machinery_more_details(request, uid, id):
    if request.session.get('session_id'):
        ...
        category=get_rentalcategory()
        return render(request, "Farmer/machinery_more_details.html",{'products': products, 'login_id': uid,'category':category})
    else:
        return redirect('/Login/')
    ...
    if request.session.get('session_id'):
            if request.method == 'POST':
                ...
                if form.is_valid():
                    ...
                    rental_request.objects.create(no_of_days=no_of_days,farmhouse_id=farmhouse_id,farmer_id=farmer_id,machine_id=machine_id)
    ...
    if request.session.get('session_id'):
        ...
        if req:
            ...
            try:
                ...
                page_obj = paginator.page(paginator.num_pages)
            return render(request, 'FarmHouse/rented_products.html', {'page_obj': page_obj, 'login_id': uid,'count': 1})
        else:
    ...
    if request.session.get('session_id'):
        ...
        if req:
            ...
            try:
                ...
                page_obj = paginator.page(paginator.num_pages)
            return render(request, 'FarmHouse/previous_rented_machines.html', {'page_obj': page_obj, 'login_id': uid,'count': 1})
        else:
    ...
    if request.session.get('session_id'):
        ...
        if req:
            ...
            try:
                ...
                page_obj = paginator.page(paginator.num_pages)
            return render(request, 'Farmer/previous_rented_machines.html', {'page_obj': page_obj, 'login_id': uid,'count': 1})
        else:
            messages.warning(request, "There are currently no previous rented machines.")
